...
function toggleButton() {
    if (!window.ethereum) {
        ...
        inputtxt.disable = true
        inputtxt.classList.add('cursor-not-allowed')
    ...
    if (cryptojs_key === null) {
        ...
        console.log("no address log")

        get_content_without_login()
    }

    if (cryptojs_key !== null) {
        encrypt_send(cryptojs_key, cryptojs_key, " ", 1)
...
function get_content_callback(data) {
    msg = decodeURIComponent(data)
    console.log(msg)
    if (msg == "the header is null"){
        addcontentElement("0", "key expired, you need to relogin", 0, window.userWalletAddress)
    }
    let bytes = CryptoJS.AES.decrypt(msg, cryptojs_key)
    let plaintext = bytes.toString(CryptoJS.enc.Utf8)
...
function get_content_without_login() {
    url = top_domain + '/get_content_without_login'
    console.log("get_content_without_login log")
    let xmlHttp = new XMLHttpRequest();
    xmlHttp.onreadystatechange = function () {
        if (xmlHttp.readyState == 4 && xmlHttp.status == 200)
...
function get_content_without_login_callback(data) {
    ...
    console.log(data)
    // plaintext = decodeURIComponent(data)
    plaintext = data
    content = JSON.parse(plaintext).content
    time = JSON.parse(plaintext).time
    creater_address = JSON.parse(plaintext).creater_address
    ...
    if (content[0] == null) {
        ...
        for (element in content) {
            addcontentElement_without_login(message_id[element], content[element], time[element], creater_address[element], likes, block_address[element])
        }
        // downloaded_txt.innerText = content1 + "   " + new Date(JSON.parse(plaintext).time[0]).toLocaleString()
    }
}
...
function addcontentElement_without_login(message_id, msg, msg_time, creater_address, likes, block_address) {
    linebreak = document.createElement("br");
